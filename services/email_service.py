...
def send_handover_email(shift):
    ...
    # Create comprehensive HTML email (all styles inline for Gmail/Outlook compatibility)
    th_style = "background-color:#e9ecef; padding:12px; text-align:left; border:1px solid #dee2e6; font-weight:bold;"
    td_style = "padding:12px; border:1px solid #dee2e6;"
...
<div style="max-width:800px; margin:0 auto; font-family:Arial,sans-serif; color:#333; line-height:1.6;">
    <h3 style="color:#495057; border-bottom:2px solid #007bff; padding-bottom:10px; text-align:center;">📋 Shift Handover Details</h3>
    <table border="0" cellpadding="0" cellspacing="0" style="border-collapse:collapse; width:100%; margin-bottom:20px;">
        <tr><th style="width:30%; {th_style}">Date</th><td style="{td_style}">{shift.date}</td></tr>
        <tr><th style="{th_style}">From</th><td style="{td_style}">{shift.current_shift_type}</td></tr>
        <tr><th style="{th_style}">To</th><td style="{td_style}">{shift.next_shift_type}</td></tr>
        <tr><th style="{th_style}">Account</th><td style="{td_style}">{account_name}</td></tr>
        <tr><th style="{th_style}">Team</th><td style="{td_style}">{team_name}</td></tr>
        <tr><th style="{th_style}">Current Shift Engineers</th><td style="{td_style}">{current_engineers}</td></tr>
        <tr><th style="{th_style}">Next Shift Engineers</th><td style="{td_style}">{next_engineers}</td></tr>
        <tr><th style="{th_style}">Submission Time</th><td style="{td_style}">{shift.submitted_at.strftime('%Y-%m-%d %H:%M:%S') if shift.submitted_at else 'Just submitted'}</td></tr>
        <tr><th style="{th_style}">Submitted By</th><td style="{td_style}"><span style="color:#0d6efd; font-weight:bold;">👤 {submitted_by}</span></td></tr>
        <tr><th style="{th_style}">Status</th><td style="{td_style}"><span style="color:#198754; font-weight:bold;">✅ Completed</span></td></tr>
    </table>
    ...
    <div style="margin-top:30px; text-align:center; padding:15px; background-color:#f8f9fa; border-top:1px solid #dee2e6; color:#6c757d; font-size:12px;">
